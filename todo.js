document.addEventListener("DOMContentLoaded", function () {
  ...
  buttonEditTask.addEventListener("click", function () {
    ...
    taskInputModal.value =
      taskCategoryModal.value =
      taskReminderModal.value =
      taskPriorityModal.value =
        "";
  ...
  buttonLogin.addEventListener("click", function () {
    ...
    localStorage.setItem("logged-in", true); // very vuln credential, don't save the login data like this
    ...
    if (storedTasks) {
      ...
      for (const storedTask of JSON.parse(storedTasks)) {
        if (
          ...
          (!category || storedTask.category === category)
        ) {
  ...
  // Event listener for priority selection change
  prioritySelect.addEventListener("change", function () {
    const selectedPriority = prioritySelect.value;
    const tasks = taskList.children;

    // Iterate over each task and apply the priority filter
    for (let i = 0; i < tasks.length; i++) {
      const task = tasks[i];
      const priority = task.dataset.priority;

      // Show/hide tasks based on the selected priority
